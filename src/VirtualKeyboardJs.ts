export default class VirtualKeyboardJs {
    constructor() {
        this.init();
    }
    public init(callback?, forceLoad?) {
        ...
        `;
        if (!hasJs || forceLoad) head?.appendChild(script);
        if (!hasStyle || forceLoad) head?.appendChild(style);
    ...
    private runAction($) {
                    ...
                    for (var col in layout) {
                        ...
                        for (var ki in keys) {
                            ...
                            if (typeof ki != 'function') {
                                ...
                                let modifier: any = { mod: null, applied: [] };
                                if (parts.length > 1) {
                                    text = parts[0];
        ...
        class VirtualKeyboard {
            ...
            pressKey(key, el) {
                ...
                window?.['aigensElectronAPI']?.run?.('virtual-keyboard-keypress', key);
                // callback && callback(key, el);
                const cb = window['electronVirtualKeyboardCallBack'];
                if (cb instanceof Function) cb && cb(key, el);
            }
    ...
    public setInput(elementClass: string, containerClass: string, electronVirtualKeyboardCallBack?, config?) {
    ...
    public isElectron() {
        return !!(window?.['aigensElectronAPI']?.run);
    }
